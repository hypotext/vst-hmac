...
Lemma first64_sixtyfour {A} (a:A): 
      firstn 64 (HMAC_progZ.HMAC_SHA256.sixtyfour a) = HMAC_progZ.HMAC_SHA256.sixtyfour a.
...
Proof. intros. repeat rewrite Zlength_correct.
   unfold HMAC_progZ.HMAC_SHA256.mkArgZ, HMAC_progZ.HMAC_SHA256.mkArg, HMAC_progZ.HMAC_SHA256.sixtyfour.
...
Lemma zeropad_isbyteZ: forall l, Forall isbyteZ l -> Forall isbyteZ (HMAC_progZ.HMAC_SHA256.zeroPad l).
Proof. unfold isbyteZ, HMAC_progZ.HMAC_SHA256.zeroPad; intros.
...
Lemma nth_zeropad_left {d d'}: forall l i (I: 0<= i < Zlength l),
      nth (Z.to_nat i) (HMAC_progZ.HMAC_SHA256.zeroPad l) d = nth (Z.to_nat i) l d'.
Proof. unfold HMAC_progZ.HMAC_SHA256.zeroPad. intros.
  destruct I.
...
Lemma mkKey_left {d d'}: forall l (L: false = (Zlength l >? 64)) 
        i (I: 0<= i < Zlength l),
      nth (Z.to_nat i) (HMAC_progZ.HMAC_SHA256.mkKey l) d = nth (Z.to_nat i) l d'.
Proof.
  unfold HMAC_progZ.HMAC_SHA256.mkKey. intros. simpl. rewrite <- L.
  apply nth_zeropad_left; trivial.
Qed.


Lemma nth_zeropad_right {d} l i (I: Zlength l <= i < 64):
      nth (Z.to_nat i) (HMAC_progZ.HMAC_SHA256.zeroPad l) d = Z0.
Proof. unfold HMAC_progZ.HMAC_SHA256.zeroPad. 
...
Lemma mkKey_right {d}: forall l (L: false = (Zlength l >? 64)) 
        i (I: Zlength l <= i < 64),
      nth (Z.to_nat i) (HMAC_progZ.HMAC_SHA256.mkKey l) d = Z0.
Proof.
  unfold HMAC_progZ.HMAC_SHA256.mkKey. intros. simpl. rewrite <- L.
...
(* HMAC_progZ.SHA256_ changed *)

Lemma zeroPad_BlockSize: forall k, (length k <= HMAC_progZ.SHA256_.BlockSize)%nat ->
  length (HMAC_progZ.HMAC_SHA256.zeroPad k) = HMAC_progZ.SHA256_.BlockSize%nat.
Proof. unfold HMAC_progZ.HMAC_SHA256.zeroPad. intros. rewrite app_length, (*length_Nlist*) length_list_repeat. omega. 
Qed. 

Lemma length_SHA256': forall l, 
  length (functional_prog.SHA_256' l) = HMAC_progZ.SHA256_.DigestLength.
...
Lemma mkKey_length l: length (HMAC_progZ.HMAC_SHA256.mkKey l) = HMAC_progZ.SHA256_.BlockSize.
Proof. intros. unfold HMAC_progZ.HMAC_SHA256.mkKey.
  remember (Zlength l >? Z.of_nat HMAC_progZ.SHA256_.BlockSize) as z. 
  ...
  apply zeroPad_BlockSize.
    ...
    apply Nat2Z.inj_le. 
    specialize (Zgt_cases (Z.of_nat (Datatypes.length l)) (Z.of_nat HMAC_progZ.SHA256_.BlockSize)). rewrite <- Heqz. trivial.
Qed.

Lemma mkKey_atBlockSize s: length s = HMAC_progZ.SHA256_.BlockSize%nat ->
      HMAC_progZ.HMAC_SHA256.mkKey s = s.
Proof. unfold HMAC_progZ.HMAC_SHA256.mkKey. rewrite Zlength_correct.
  intros. rewrite H.
  simpl. unfold HMAC_progZ.HMAC_SHA256.zeroPad. rewrite H. simpl.
  apply app_nil_r.  
Qed.
...
Lemma HMAC_length d k: length (HMAC_progZ.HMAC256 d k) = 32%nat.
Proof.
  unfold HMAC_progZ.HMAC_SHA256.HMAC, HMAC_progZ.HMAC_SHA256.OUTER.
  apply length_SHA256'.
Qed.
Lemma HMAC_Zlength d k: Zlength (HMAC_progZ.HMAC256 d k) = 32.
Proof.
